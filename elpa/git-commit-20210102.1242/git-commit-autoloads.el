...
or call the function `global-git-commit-mode'." :set #'custom-set-minor-mode :initialize (lambda (symbol exp) (custom-initialize-default symbol exp) (when global-git-commit-mode (add-hook 'find-file-hook 'git-commit-setup-check-buffer))) :group 'git-commit :type 'boolean)
