...
PROG=$( basename "$0" )
TEMP=$( getopt --options h --longoptions termenv:,title:,class:,working-directory:,tmux-session:,help -- "$@" ) || exit 1
eval set -- "$TEMP"
...
if [[ $working_directory =~ $regex ]]; then
    ...
    if [ -z "$other_args" ]; then
        cmd="$termenv ssh -t $userat$host \"cd $path && exec "'\$SHELL'"\" && exec $SHELL"
    else
        cmd="$termenv ssh -t $userat$host \"cd $path &&  $other_args; \$SHELL -i -l\" "
    fi
    ...
    path=${BASH_REMATCH[5]}
    if [ -z "$other_args" ]; then
        cmd="$termenv ssh -t $userat$host -p $port \"cd $path && exec "'\$SHELL'"\" && exec $SHELL"
    else
        cmd="$termenv ssh -t $userat$host -p $port \"cd $path &&  $other_args; \$SHELL -i -l\" "
    fi
    if [ -z "$tmux_session" ]; then
        eval  $TERMINAL $term_args $TERMINAL_EXEC  $SHELL  -i -c \'$cmd\'
    else
