...
(setq-default
 ...
 frame-title-format '( (:eval (upcase (char-to-string (aref  (symbol-name helix--current-state) 0))) ) "「" mode-line-buffer-identification "」["  (:propertize ("" mode-name) ) "] "    mode-line-misc-info   " GNU/Emacs<" (:eval (concat hostname (expand-file-name default-directory))) ">")
...
(defun vmacs-calc-hook()
  (require 'calc-bin)
  ;; (meep-local-set-key "/" #'calc-divide)
