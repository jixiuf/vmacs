...
(defun term-compile ()
  ...
  (if (eq system-type 'darwin)
    ...
    (call-process "sh" nil nil nil "-c"
                  (format "hypr-run-or-raise --cd --floating 'foot|dterm|Alacritty|kitty|org.wezfurlong.wezterm' -- term.sh  --working-directory=$(hypr-cwd||echo $HOME) --class=dterm -- tmux.sh --session dterm --cwd $(hypr-cwd||echo $HOME);tmux send-keys C-u C-l '%s' Enter" compile-command))
...
(defun compile-dwim-locate-makefile ()
...
;; 15 =R
;; 11=B
(defun compile-dwim-xcode:build-and-run()
  (interactive)
...
key code 15 using {command down}
    ...
    (start-process "compile-dwim-xcode:build-and-run" nil "osascript" "-e" cmd)))

(defun compile-dwim-xcode:build()
  (interactive)
  (let ((cmd  "tell application \"Xcode\" to activate
tell application \"System Events\"
tell process \"Xcode\"
