...
(defcustom compile-dwim-alist
  `(
    ...
    (pimacs-chat (mode . pimacs-chat-mode)
          pimacs-resume
          pimacs-resume
          )
    (objc (mode . objc-mode)
...
(defun compile-dwim-run ()
  ...
  (if (and (not (buffer-file-name))
           ...
           (not (derived-mode-p 'vc-dir-mode))
           (not (derived-mode-p 'log-view-mode))
           (not (derived-mode-p 'pimacs-chat-mode))
           (not (derived-mode-p 'magit-mode))
           (not (eq major-mode 'dired-mode)))
      (call-interactively 'term-compile)
    (compile-dwim-make-local-vars)
...
(defun compile-go-test-current()
  ...
  (let ((funname (which-function)))
    ...
    (if (string-prefix-p "Bench" funname)
        (setq compile-command (format "go test -v -bench=%s  -benchmem  -count=1 -test.run ^%s$"  funname funname))
